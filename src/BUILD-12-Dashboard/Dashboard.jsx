...
import { Home, TrendingUp, DollarSign, Package, AlertTriangle, ShoppingCart, Clock, XCircle, RefreshCw, Activity, Users, BarChart3, CheckCircle } from 'lucide-react';
import { salesAPI, stockAPI, medicinesAPI, paymentsAPI, suppliersAPI } from '../services/api';
...
export default function DashboardApp({ appData, setAppData, onNavigate }) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [salesRes, stockRes, medicinesRes, paymentsRes, suppliersRes] = await Promise.all([
        salesAPI.getAll(),
        stockAPI.getAll(),
        medicinesAPI.getAll(),
        paymentsAPI.getAll(),
  ...
  const stats = useMemo(() => {
    ...
    const monthlyRevenue = thisMonthSales.reduce((sum, s) => sum + parseFloat(s.final_amount || 0), 0);
  ...
  }, [sales, stock, medicines, payments, suppliers]);

  // Calculate system health score
  const systemHealth = useMemo(() => {
    let score = 100;
    if (stats.expiredCount > 0) score -= 20;
    if (stats.expiringSoonCount > 5) score -= 15;
    if (stats.outOfStockCount > 3) score -= 15;
    if (stats.overduePayments > 0) score -= 20;
    if (stats.lowStockCount > 10) score -= 10;
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={loadData}
                disabled={loading}
                className="flex items-center gap-2 px-4 py-2 bg-white text-blue-600 rounded-lg hover:bg-blue-50 transition-colors disabled:opacity-50 shadow"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <MetricCard
            ...
            subtitle={`₹${stats.totalPendingAmount.toFixed(0)}`}
            icon={DollarSign}
            color="bg-orange-600"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
            ...
            <div className="p-6 space-y-3">
              ...
              <AlertItem
                label="Total Pending"
                count={stats.totalPendingPayments}
                color="bg-yellow-600"
                icon={DollarSign}
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
          ...
          {sales.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg">No recent sales</p>
              <button
                onClick={() => onNavigate && onNavigate('pos')}
            ...
            <div className="space-y-3">
              {sales.slice(0, 5).map(sale => (
                <div key={sale.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg hover:shadow-md transition-all border border-gray-100">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-green-600 text-lg">₹{parseFloat(sale.final_amount).toFixed(2)}</p>
...
function MetricCard({ title, value, subtitle, icon: Icon, color, onClick }) {
  return (
    <div 
      onClick={onClick}
      className="bg-white rounded-xl shadow-lg p-6 cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all border border-gray-100"
    >
...
function AlertItem({ label, count, color, icon: Icon, amount, onClick }) {
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className={`${color} p-2 rounded-lg shadow-sm`}>
...
function QuickActionButton({ icon: Icon, label, color, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`${color} text-white p-6 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col items-center justify-center gap-3`}
    >
