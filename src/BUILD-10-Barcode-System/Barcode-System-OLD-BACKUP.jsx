...
import { Camera, Printer, Search, Download, Settings, Zap, Package, ScanLine, QrCode, Edit2, Save, X, CheckCircle, AlertCircle, Grid, List, Hash } from 'lucide-react';

// ==================== MOCK DATA ====================

const SAMPLE_MEDICINES = [
  { id: 1, name: 'Paracetamol', genericName: 'Acetaminophen', company: 'Cipla Ltd', packSize: '10 tablets', mrp: 12.50, purchasePrice: 8.00, barcode: 'MED000001', category: 'Analgesic' },
  { id: 2, name: 'Crocin', genericName: 'Paracetamol', company: 'GSK', packSize: '15 tablets', mrp: 15.00, purchasePrice: 10.50, barcode: 'MED000002', category: 'Antipyretic' },
  { id: 3, name: 'Amoxicillin', genericName: 'Amoxicillin Trihydrate', company: 'Sun Pharma', packSize: '10 capsules', mrp: 45.00, purchasePrice: 32.00, barcode: 'MED000003', category: 'Antibiotic' },
  { id: 4, name: 'Cetirizine', genericName: 'Cetirizine Hydrochloride', company: 'Dr. Reddy\'s', packSize: '10 tablets', mrp: 18.00, purchasePrice: 12.00, barcode: 'MED000004', category: 'Antihistamine' },
  { id: 5, name: 'Omeprazole', genericName: 'Omeprazole', company: 'Lupin', packSize: '14 capsules', mrp: 55.00, purchasePrice: 38.00, barcode: 'MED000005', category: 'Antacid' },
];
  ...
  const generateAllBarcodes = () => {
    ...
    const updated = medicines.map(m => {
      if (!m.barcode || m.barcode === '') {
        count++;
        return { ...m, barcode: `MED${String(m.id).padStart(6, '0')}` };
      }
      return m;
    });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">BUILD 10 - Scan, Generate & Print Barcodes</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <div className="mt-3 max-h-60 overflow-y-auto border rounded-lg">
                  {filteredMedicines.map(med => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-500">Barcode: {med.barcode}</div>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-gray-600 mb-4">{scanResult.genericName}</p>
                
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Company:</span>
                    <span className="font-semibold">{scanResult.company}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{scanResult.packSize}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">MRP:</span>
                    <span className="font-bold text-green-600 text-xl">₹{scanResult.mrp.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Purchase Price:</span>
                    <span className="font-semibold">₹{scanResult.purchasePrice.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <tbody className="divide-y">
                {medicines.map(medicine => (
                  <tr key={medicine.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">{medicine.company}</td>
...
function MedicineCard({ medicine, format, onGenerate }) {
  return (
    <div className="bg-white rounded-lg border-2 border-gray-200 p-4 hover:border-blue-300 hover:shadow-md transition-all">
      ...
      <p className="text-sm text-gray-600 mb-3">{medicine.company}</p>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm font-mono text-gray-500 mt-1">{medicine.barcode}</p>
                </div>
              </div>
            </label>
          ))}
            ...
            <div className="grid grid-cols-3 gap-4">
              {selectedMeds.map(medicine => (
                <div key={medicine.id} className="border-2 border-dashed border-gray-300 p-3 rounded bg-white">
                  ...
                  {includePrice && (
                    <div className="font-bold text-sm text-center mt-1">MRP: ₹{medicine.mrp.toFixed(2)}</div>
                  )}
                </div>
              ))}
            </div>
...
function BarcodeManager({ medicines, setMedicines, showNotification }) {
              ...
              <tr key={medicine.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  {editingId === medicine.id ? (
                    ...
                    <span className="font-mono font-bold">{medicine.barcode}</span>
                  )}
                </td>
                <td className="px-6 py-4">
